...
function loadCart(
  ...
  (function ($) {
          ...
          if (cartPricesLoaded[0]) {
            ...
            startParts.forEach((elm) => {
              if (elm.isVisible) {
                if (elm.material.diffuseColor.r > 0.3) {
                  startEndCartSilb += elm.scaling.x;
                } else {
                  startEndCartAnth += elm.scaling.x;
                }
              }
            });
            startEndCartSilb = Math.ceil(startEndCartSilb);
            startEndCartAnth = Math.ceil(startEndCartAnth);
              ...
              for (let j = 0; j < fenceBoards[i].length; j++) {
                if (fenceBoards[i][j].isVisible) {
                  if (fenceBoards[i][j].material.diffuseColor.b < 0.3) {
                    fenceBoardsCartAnth += fenceBoards[i][j].scaling.x;
                  } else if (
                    fenceBoards[i][j].material.diffuseColor.b > 0.5 &&
                    fenceBoards[i][j].material.diffuseColor.b < 0.8
                  ) {
                    fenceBoardsCartGrau += fenceBoards[i][j].scaling.x
                  } else if (fenceBoards[i][j].material.diffuseColor.b > 0.8) {
                    fenceBoardsCartSand += fenceBoards[i][j].scaling.x;
                  } else if (
                    fenceBoards[i][j].material.diffuseColor.b > 0.3 &&
                    fenceBoards[i][j].material.diffuseColor.b < 0.5
                  ) {
                    fenceBoardsCartBraun += fenceBoards[i][j].scaling.x;
                  }
                }
              }
            }
              ...
              if (smallBoardsArr[i].isVisible) {
                if (smallBoardsArr[i].material.diffuseColor.r > 0.3) {
                  if (fencesArr[i].size > 60) {
                    smallBoardLongSilberCart += smallBoardsArr[i].scaling.x;
                  } else {
                    smallBoardShortSilberCart += smallBoardsArr[i].scaling.x * 3;
                  }
                } else {
                  if (fencesArr[i].size > 60) {
                    smallBoardLongAnthCart += smallBoardsArr[i].scaling.x;
                  } else {
                    smallBoardShortAnthCart += smallBoardsArr[i].scaling.x * 3;
                  }
                }
              }
            }
            smallBoardLongAnthCart = Math.ceil(smallBoardLongAnthCart);
            smallBoardLongSilberCart = Math.ceil(smallBoardLongSilberCart);
            smallBoardShortAnthCart = Math.ceil(smallBoardShortAnthCart);
            smallBoardShortSilberCart = Math.ceil(smallBoardShortSilberCart);
            //populate desighn inlays
            let inlaysVieroSilberCart = 0;
            let inlaysVieroAnthCart = 0;
            let inlaysAstroSilberCart = 0;
            let inlaysAstroAnthCart = 0;
            let inlaysSnowSilberCart = 0;
            let inlaysSnowAnthCart = 0;
            for (let i = 0; i < inlays.length; i++) {
              if (inlays[i][1].isVisible) {
                if (inlays[i][1].material.albedoColor.r > 0.5)
                  inlaysVieroSilberCart += inlays[i][1].scaling.x;
                if (inlays[i][1].material.albedoColor.r < 0.5)
                  inlaysVieroAnthCart += inlays[i][1].scaling.x;
              }
              if (inlays[i][2].isVisible) {
                if (inlays[i][2].material.albedoColor.r > 0.5)
                  inlaysAstroSilberCart += inlays[i][2].scaling.x;
                if (inlays[i][2].material.albedoColor.r < 0.5)
                  inlaysAstroAnthCart += inlays[i][2].scaling.x;
              }
              if (inlays[i][3].isVisible) {
                if (inlays[i][3].material.albedoColor.r > 0.5)
                  inlaysSnowSilberCart += inlays[i][3].scaling.x;
                if (inlays[i][3].material.albedoColor.r < 0.5)
                  inlaysSnowAnthCart += inlays[i][3].scaling.x;
              }
            }
            inlaysVieroSilberCart = Math.ceil(inlaysVieroSilberCart);
            inlaysVieroAnthCart = Math.ceil(inlaysVieroAnthCart);
            inlaysAstroSilberCart = Math.ceil(inlaysAstroSilberCart);
            inlaysAstroAnthCart = Math.ceil(inlaysAstroAnthCart);
            inlaysSnowSilberCart = Math.ceil(inlaysSnowSilberCart);
            inlaysSnowAnthCart = Math.ceil(inlaysSnowAnthCart);
            //populate laisne
            ...
            for (let i = 0; i < laisnes.length; i++) {
              laisnes[i].forEach((elm) => {
               if (elm.isVisible) {
                  if (elm.material.diffuseColor.r > 0.3) {
                    laisneSilberCart += elm.scaling.x;
                  } else {
                    laisneAnthCart += elm.scaling.x;
                  }
                }
              });
            }
            laisneSilberCart = Math.ceil(laisneSilberCart);
            laisneAnthCart = Math.ceil(laisneAnthCart);
            ...
            for (let i = 0; i < gardoFenceBoards.length; i++) {
              ...
              for (let j = 0; j < gardoFenceBoards[i].length; j++) {
                if (gardoFenceBoards[i][j].isVisible) {
                  fenceBoardsCart += gardoFenceBoards[i][j].scaling.x;
                }
              }
            }
            ...
            woodTopParts.forEach((elm) => {
              if (elm.isVisible) {
                woodTopPartsCart += elm.scaling.x;
              }
            });
            woodTopPartsCart = Math.ceil(woodTopPartsCart);
            ...
            cartItems.push(
              ...
              ledsCabelCart,
              ledsConectorCart,
              inlaysAstroSilberCart,
              inlaysAstroAnthCart,
              inlaysSnowSilberCart,
              inlaysSnowAnthCart,
              fenceBoardsCart,
              woodTopPartsCart,
              rankelementsCart
    ...
    $.fn.smartCart.defaults = {
      ...
      cartItemTemplate:
        "<small>Art. <%=pid%></small><strong><%=pname%></strong><small><%=pabm%></small>",
      // Events
