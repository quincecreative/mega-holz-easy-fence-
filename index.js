...
var createScene = function () {
  ...
  fencePartsColors = ["#cccccc", "#474747"];
  ...
  //SHARE
  let leftPostSharedSize = 0;
  let leftSturmFromShare = 0;
  let mainLedFromShare = 0;
  let mainHauswandFromShare = 0;
  // if (window.location.href.includes("?") && !activeFence) {
  if (window.parent.location.href.includes("?") && !activeFence) {
    // let sharedLinkParts = window.location.href.split("?");
    let sharedLinkParts = window.parent.location.href.split("?");

    let propsDecoded = decodeURIComponent(sharedLinkParts[1]);
    propsDecoded = JSON.parse(propsDecoded);
    fencePostMat.diffuseColor = BABYLON.Color3.FromHexString(
      fencePartsColors[propsDecoded[0][4]]
    );
    leftPostSharedSize = propsDecoded[0][19];
    leftSturmFromShare = propsDecoded[0][20];
    mainLedFromShare = propsDecoded[0][21];
    ledMat.diffuseColor = ledMat.emissiveColor = propsDecoded[0][22];
    mainHauswandFromShare = propsDecoded[0][23];
  }
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    startParts[activeFence].scaling.x =
      ...
      inlays[activeFence][0].scaling.x =
        fenceScale;
    inlays[activeFence][1].scaling.x = fenceScale;
    inlays[activeFence][2].scaling.x = fenceScale;
    inlays[activeFence][3].scaling.x = fenceScale;
    startParts[activeFence].position.x = endParts[activeFence].position.x =
      -0.9 + fenceSize / 2 - 0.01;
    inlays[activeFence][0].position.x =
      inlays[activeFence][1].position.x =
      inlays[activeFence][2].position.x =
      inlays[activeFence][3].position.x =
  ...
  BABYLON.SceneLoader.ImportMeshAsync(
    "",
    "mesh/",
    // "https://mega-holz.de/zaunplaner/fence-configurator-assets/easy-fence/mesh/",
    "mainPost.glb"
  ).then(
    (result) => {
      ...
      roots.unshift(leftRoot0, leftRoot1);
      ...
      singsDel.unshift(signPlaneDelLeft);
      ...
      sturVorderseiteSrafs.unshift(leftStrVordSraf);
      ...
      sturRuckseiteSrafs.unshift(leftStrRuckSraf);
      ...
      // no sturmanker 0, vord anth 1, vord silb 2, ruck anth 3, ruck silb 4
      //SHARE
      if (leftSturmFromShare == 1) {
        leftStrVord.isVisible = true;
        foundationLeftStart.isVisible = false;
        foundationLeft.isVisible = false;
        foundationVordStart.isVisible = true;
        foundationVord.isVisible = true;
      }
      // setActivnesStyle(fenceSturmankerParts, 5, 0, "active-text-color");
      if (leftSturmFromShare == 2) {
        leftStrRuck.isVisible = true;
        foundationLeftStart.isVisible = false;
        foundationLeft.isVisible = false;
        foundationRuckStart.isVisible = true;
        foundationRuck.isVisible = true;
      }
      ...
      //SHARE CAP
      if (window.parent.location.href.includes("?")) {
        if (leftPostSharedSize == 1) {
          leftPost.scaling.y = 1.2;
          leftPost.position.y = 0.7717;

          leftRoot0.isVisible = false;
          leftRoot1.isVisible = false;

          foundationLeft.scaling.y = 1;
          foundationVord.scaling.z = 1;
          foundationRuck.scaling.z = 1;

          foundationLeft.position.y = -0.261;
          foundationVord.position.z = 0.261;
          foundationRuck.position.z = 0.261;
          leftPostCaps[0].position.y = 0.961;
        }
        // setbefePfosten(1.2, 0.7717, false, 1, -0.5 / 2);
        if (leftPostSharedSize == 2) {
          leftPost.scaling.y = 1.475;
          leftPost.position.y = 0.511;
          leftRoot0.isVisible = false;
          leftRoot1.isVisible = false;
          foundationLeft.scaling.y = 1.8;
          foundationVord.scaling.z = 1.8;
          foundationRuck.scaling.z = 1.8;

          foundationLeft.position.y = -0.461;
          foundationVord.position.z = 0.461;
          foundationRuck.position.z = 0.461;
          leftPostCaps[0].position.y = 0.961;
        }
        if (leftPostSharedSize == 3) {
          leftPost.scaling.y = 0.524;
          leftPost.position.y = 0.504;
          // leftPostCap.isVisible = false;
          // leftPostCapClone.isVisible = true;
          leftPostCaps[0].position.y = 0.052;
        }
        if (leftPostSharedSize == 4) {
          leftPost.scaling.y = 0.724;
          leftPost.position.y = 0.3119;
          leftRoot0.isVisible = false;
          leftRoot1.isVisible = false;
          foundationLeft.scaling.y = 1;
          foundationVord.scaling.z = 1;
        ...
        if (leftPostSharedSize == 5) {
          leftPost.scaling.y = 0.999;
          leftPost.position.y = 0.053;
          leftRoot0.isVisible = false;
          leftRoot1.isVisible = false;
          foundationLeft.scaling.y = 1.8;
          foundationVord.scaling.z = 1.8;
  ...
  var createRightFence = (posX, posZ, rotY, type, smCol, inlaysOnOff) =>
    BABYLON.SceneLoader.ImportMeshAsync(
      ...
      /////////////////////////////////////////
      //SHARE/////////////////////////////////////
      let rightPostSharedSize = 0;
      let sturmFromShare = 0;
      let boardColorShare = 0;
      let laisneShare;
      let laisneColorShare = 0;
      let inlaysShare = 0;
      let ledsShare = 0;
      let numOfBoardsShare = 8;
      let hauswandShare = 0;
      let sizeShare = 180;
      /////
      // if (window.location.href.includes("?") && !activeFence) {
      if (window.parent.location.href.includes("?") && !activeFence) {
        ...
        let propsDecoded = decodeURIComponent(sharedLinkParts[1]);
        propsDecoded = JSON.parse(propsDecoded);
        fence.position.x = propsDecoded[fenceIdCount + 1][0];
        fence.position.z = propsDecoded[fenceIdCount + 1][1];
        fence.rotation.y = propsDecoded[fenceIdCount + 1][2];
        type = propsDecoded[fenceIdCount + 1][3];
        rightPostSharedSize = propsDecoded[fenceIdCount + 1][5];
        sturmFromShare = propsDecoded[fenceIdCount + 1][6];
        startUndAbschMat = propsDecoded[fenceIdCount + 1][9];
        boardColorShare = propsDecoded[fenceIdCount + 1][10];
        laisneShare = propsDecoded[fenceIdCount + 1][11];
        laisneColorShare = propsDecoded[fenceIdCount + 1][12];
        inlaysShare = propsDecoded[fenceIdCount + 1][13];
        ledsShare = propsDecoded[fenceIdCount + 1][14];
        numOfBoardsShare = propsDecoded[fenceIdCount + 1][15];
        hauswandShare = propsDecoded[fenceIdCount + 1][17];
        sizeShare = propsDecoded[fenceIdCount + 1][18];
      }
      //MINI/////////////////////////////////
      else if (localStorage.getItem("coordinates") != null && !activeFence) {
      ...
      function toActiveFence() {
        ...
        deleteFencePart.onclick = () => {
          if (activeFence > 0) {
            ...
            aaa = 0;
            inlays.forEach((elm) => {
              if (elm[1].isVisible || elm[2].isVisible || elm[3].isVisible) {
                aaa += 1;
              }
            });
            if (aaa < 1) {
      ...
      if (!window.parent.location.href.includes("?")) {
        ...
        newBoarsdArr.forEach((elm) => {
          elm.position.x -= 0.01;
          elm.material = fenceBoardMat;
          if (boardColorShare == 0) {
            elm.material = grauMat;
            boardColObj = "grau";
          }
          if (boardColorShare == 1) {
            elm.material = anthrazitMat;
            boardColObj = "anthrazit";
          }
          if (boardColorShare == 2) {
            elm.material = braunMat;
            boardColObj = "braun";
          }
          if (boardColorShare == 3) {
            elm.material = sandMat;
            boardColObj = "sand";
          }
        });
      ...
      //INLAYS
      let inlaysFrame = result.meshes[22];
      inlaysFrame.position.x -= 0.02;
      inlaysFrame.isVisible = false;
      inlaysFrame.material = inlaysMat;
      // fenceBoards[6].isVisible = false;
      let inlaysViero = result.meshes[25];
      inlaysViero.position.x -= 0.02;
      inlaysViero.isVisible = false;
      inlaysMaterials.push(inlaysViero.material);

      let inlaysAstro = result.meshes[24];
      inlaysAstro.position.x -= 0.02;
      inlaysAstro.isVisible = false;

      let inlaysSnow = result.meshes[23];
      inlaysSnow.position.x -= 0.02;
      inlaysSnow.isVisible = false;
      inlaysSnow.material.albedoColor = BABYLON.Color3.FromHexString("#ffffff");
      // inlaysSnow.material = inlaysMat;

      var newInlaysArr = new Array(
        inlaysFrame,
        inlaysViero,
        inlaysAstro,
        inlaysSnow
      ...
      // inlays[i][0].material.diffuseColor = BABYLON.Color3.FromHexString(d);
      //   inlays[i][a].material.albedoColor = BABYLON.Color3.FromHexString(d);

      // } else if (i == 1) {
      //   inlaysFunction(1, 2, 3, "#e6e6e6", "easyFenceInlaysSilber");
      // } else if (i == 2) {
      //   inlaysFunction(1, 2, 3, "#474747", "easyFenceInlaysAnth");
      // } else if (i == 3) {
      //   inlaysFunction(2, 1, 3, "#e6e6e6", "easyFenceInlaysSilber");
      // } else if (i == 4) {
      //   inlaysFunction(2, 1, 3, "#474747", "easyFenceInlaysAnth");
      // } else if (i == 5) {
      //   inlaysFunction(3, 1, 2, "#e6e6e6", "easyFenceInlaysSilber");
      // } else if (i == 6) {
      //   inlaysFunction(3, 1, 2, "#474747", "easyFenceInlaysAnth");
      // }
      if (window.parent.location.href.includes("?")) {
        if (inlaysShare == 1) {
          newBoarsdArr[6].isVisible = false;
          inlaysViero.isVisible = true;
          inlaysFrame.isVisible = true;
          inlaysFrame.material = silberMat;
          inlaysViero.material.albedoColor = silberMat.diffuseColor;
        }
        if (inlaysShare == 2) {
          newBoarsdArr[6].isVisible = false;
          inlaysViero.isVisible = true;
          inlaysFrame.isVisible = true;
          inlaysFrame.material = anthrazitMat;
          inlaysViero.material.albedoColor = anthrazitMat.diffuseColor;
        }
        if (inlaysShare == 3) {
          newBoarsdArr[6].isVisible = false;
          inlaysAstro.isVisible = true;
          inlaysFrame.isVisible = true;
          inlaysFrame.material = silberMat;
          inlaysAstro.material.albedoColor = silberMat.diffuseColor;
        }
        if (inlaysShare == 4) {
          newBoarsdArr[6].isVisible = false;
          inlaysAstro.isVisible = true;
          inlaysFrame.isVisible = true;
          inlaysFrame.material = anthrazitMat;
          inlaysAstro.material.albedoColor = anthrazitMat.diffuseColor;
        }
        if (inlaysShare == 5) {
          newBoarsdArr[6].isVisible = false;
          inlaysSnow.isVisible = true;
      ...
      if (!window.parent.location.href.includes("?")) {
        ...
        if (laisneColorShare == 1) {
          laisneOrg.material = anthrazitMat;
          laisnesColObj = "anthrazit";
        }
        if (laisneColorShare == 0) {
          laisneOrg.material = silberMat;
          laisnesColObj = "silber";
        }
      }
        ...
        for (let i = 0; i < 7; i++) {
          ...
          if (!window.parent.location.href.includes("?")) {
            if (checkboxActive[i]) {
              ...
              if (i < 6) {
                inlaysFrame.position.y += 0.01;
                inlaysViero.position.y += 0.01;
                inlaysAstro.position.y += 0.01;
            ...
            //share size
            fenceScale = sizeShare / 180;
            fenceSize = 1.8 * fenceScale;
            newLaisnesArr.forEach((elm) => {
              elm.scaling.x = fenceScale;
              elm.position.x = -0.9 + fenceSize / 2 - 0.01;
            });
          }
      ...
      //SHARE
      if (sturmFromShare == 1) {
        rightStrVord.isVisible = true;
        foundationRightStart.isVisible = false;
        foundationRight.isVisible = false;
        foundationVordStart.isVisible = true;
        foundationVord.isVisible = true;
      }
      // setActivnesStyle(fenceSturmankerParts, 5, 0, "active-text-color");
      if (sturmFromShare == 2) {
        rightStrRuck.isVisible = true;
        foundationRightStart.isVisible = false;
        foundationRight.isVisible = false;
        foundationRuckStart.isVisible = true;
        foundationRuck.isVisible = true;
      ...
      //SHARE CAP
      if (window.parent.location.href.includes("?")) {
        if (rightPostSharedSize == 0) {
          rightPost.scaling.z = 1;
          rightPost.position.y = 0.961;

          rightRoot0.isVisible = true;
          rightRoot1.isVisible = true;
          ...
          rightPostCap.isVisible = true;
          rightPostCapClone.isVisible = false;
        }
        if (rightPostSharedSize == 1) {
          rightPost.scaling.z = 1.2;
          rightPost.position.y = 0.7717;
      ...
      //share main hauswand
      if (mainHauswandFromShare == 1) addRemoveHauswand();

      //set share easyFenceHalf
      if (type == "easyFenceHalf") {
        newBoarsdArr[7].isVisible = false;
        newBoarsdArr[6].isVisible = false;
        newBoarsdArr[5].isVisible = false;
        newBoarsdArr[4].isVisible = false;
        startPart.position.y = newBoarsdArr[3].position.y + 0.23 / 2 + 0.005;
      }

      //share num of boards
      let startNumOfBoards = 8;
      setTimeout(() => {
        if (window.parent.location.href.includes("?")) {
          while (startNumOfBoards > numOfBoardsShare) {
            startNumOfBoards -= 1;
            if (startNumOfBoards > 1) {
              newBoarsdArr[startNumOfBoards].isVisible = false;
              if (laisneShare[startNumOfBoards - 1])
                newLaisnesArr[startNumOfBoards - 1].isVisible = false;
              if (
                (type == "easyFenceInlaysAnth" && startNumOfBoards == 6) ||
                (type == "easyFenceInlaysSilber" && startNumOfBoards == 6)
              ) {
                newInlaysArr[0].isVisible = false;
                newInlaysArr[1].isVisible = false;
                newInlaysArr[2].isVisible = false;
                newInlaysArr[3].isVisible = false;
              }
              startPart.position.y =
                newBoarsdArr[startNumOfBoards - 1].position.y +
                0.23 / 2 +
                0.005;
            }
          }
      ...
      if (hauswandShare == 1) {
        directeHauswandMeshesRight[
          directeHauswandMeshesRight.length - 1
        ].isVisible = true;
        newRootsArr.forEach((elm) => {
          elm.isVisible = false;
        });
        foundationRightStart.isVisible = false;
        foundationRight.isVisible = false;

        rightPost.scaling.x = rightPostCap.scaling.x = 0.7;
        rightPost.scaling.y = rightPostCap.scaling.y = 0.7;

        directeHauswandMeshesRight[
          directeHauswandMeshesRight.length - 1
        ].scaling.z = directeHauswandMeshesRight[
          directeHauswandMeshesRight.length - 1
        ].scaling.x = 1.43;
        directeHauswandMeshesRight[
          directeHauswandMeshesRight.length - 1
      ...
      if (window.parent.location.href.includes("?")) {
        ...
        startPart.scaling.x =
          endPart.scaling.x =
          newInlaysArr[0].scaling.x =
            fenceScale;
        newInlaysArr[1].scaling.x = fenceScale;
        newInlaysArr[2].scaling.x = fenceScale;
        newInlaysArr[3].scaling.x = fenceScale;

        startPart.position.x = endPart.position.x = -0.9 + fenceSize / 2 - 0.01;
        newInlaysArr[0].position.x =
          newInlaysArr[1].position.x =
          newInlaysArr[2].position.x =
          newInlaysArr[3].position.x =
            -0.9 + fenceSize / 2 - 0.02;

        smallBoards.scaling.x = fenceScale;
        smallBoards.position.x = -0.9 + fenceSize / 2 - 0.01;
        rightPost.position.x = -0.9 + fenceSize;
        rightPostCap.position.x = rightPost.position.x;
        ...
        //sa set timeout probaj
        newFenceForwardSigns[newFenceBackSigns.length - 1].setAbsolutePosition(
          new BABYLON.Vector3(
            getAbsPosX(rightPost) + 0.3,
            newFenceForwardSigns[newFenceBackSigns.length - 1].position.y,
            getAbsPosZ(rightPost)
          )
        );

        newFenceRightSigns[newFenceBackSigns.length - 1].setAbsolutePosition(
          new BABYLON.Vector3(
            getAbsPosX(rightPost),
            newFenceRightSigns[newFenceBackSigns.length - 1].position.y,
            getAbsPosZ(rightPost) - 0.3
          )
        );

        newFenceLeftSigns[newFenceBackSigns.length - 1].setAbsolutePosition(
          new BABYLON.Vector3(
            getAbsPosX(rightPost),
            newFenceLeftSigns[newFenceBackSigns.length - 1].position.y,
            getAbsPosZ(rightPost) + 0.3
          )
        );
      ...
      if (!window.parent.location.href.includes("?")) {
        ...
        for (let i = 0; i < laisneShare.length; i++) {
          laisnesThis.push(laisneShare[i]);
        }
      }

      !window.parent.location.href.includes("?")
        ? (numOfBoards = 8)
        : (numOfBoards = numOfBoardsShare);
      ...
      if (window.parent.location.href.includes("?") && !activeFence) {
        ...
        if (fenceId == 0) {
          //new share
          fencesArr[0].children = propsDecoded[0][8];
          ////////
          fencesArr[0].parent = undefined;
        }
        if (fenceId > 0) {
          //new share
          fencesArr[fenceId].children = propsDecoded[fenceId][8];
          ////////

          //share delete parent
          if (propsDecoded[fenceId][7] != null) {
            fencesArr[fenceId].parent = propsDecoded[fenceId][7];
          } else {
            fencesArr[fenceId].parent = undefined;
          }
          ////////
        }
        if (fenceId === propsDecoded.length - 1) {
          setTimeout(() => {
            //share id
            // fencesArr.forEach((fence) => {
            for (let a = 0; a < fencesArr.length; a++) {
              //share delete parent
              if (fencesArr[a].parent === "deleted") {
                //////
                ...
                inlays[a][1].isVisible = false;
                inlays[a][2].isVisible = false;
                inlays[a][3].isVisible = false;
            ...
            /////
            history.pushState(null, "", parent.location.href.split("?")[0]);
          }, 2000);
        }
      }
      ///////////////////////////////////////////////////////////////////////////////////
  ...
  if (window.parent.location.href.includes("?")) {
    //let sharedLinkPartsBuild = window.location.href.split("?");
    let sharedLinkPartsBuild = window.parent.location.href.split("?");

    let propsDecodedBuild = decodeURIComponent(sharedLinkPartsBuild[1]);
    propsDecodedBuild = JSON.parse(propsDecodedBuild);

    //CREATE DEFAULT FENCE
    function handleTabActivnesShare() {
      if (!document.hidden) {
        for (let i = 0; i < propsDecodedBuild.length; i++) {
          createRightFence(
            propsDecodedBuild[i][0],
            propsDecodedBuild[i][1],
            propsDecodedBuild[i][2],
            propsDecodedBuild[i][3],
            propsDecodedBuild[i][16],
            0
          );
        }
        clearInterval(refreshIntervalIdShare);
      }
    }

    if (document.hidden) {
      var refreshIntervalIdShare = setInterval(handleTabActivnesShare, 100);
    } else {
      for (let i = 0; i < propsDecodedBuild.length; i++) {
        createRightFence(
          propsDecodedBuild[i][0],
    ...
    function handleTabActivnes() {
      if (!document.hidden) {
        if (localStorage.getItem("coordinates") === null) {
          ...
          console.log(fencesCoordinates[0].length)
          if (fencesCoordinates[0].length < 6) {
    ...
    if (fencesArr[j].fenceGardo == undefined) {
      if (rightPosts[j].isVisible) {
        ...
        if (laisnePos < 6) {
          ...
          inlays[j][1].position.y += 0.01;
          inlays[j][2].position.y += 0.01;
          inlays[j][3].position.y += 0.01;
        }
  ...
  setPartsAndconf(designInlaysSecond, false, fencePartsColors);
  // //third inlay setings
  let designInlaysThird = document.getElementsByClassName(
    "third-set-design-inlays-color"
  );
  setPartsAndconf(designInlaysThird, false, fencePartsColors);

  //inlays show or not
  function inlaysFunction(a, b, c, d, e) {
    for (let i = 0; i < rightPosts.length; i++) {
      if (rightPosts[i].isVisible && fencesArr[i].fenceGardo == undefined) {
        inlays[i][0].isVisible = true;
        inlays[i][a].isVisible = true;
        inlays[i][b].isVisible = false;
        inlays[i][c].isVisible = false;
    ...
    for (let i = 0; i < inlays.length; i++) {
      inlays[i][0].material = d;
      if (a != 3) inlays[i][a].material.albedoColor = d.diffuseColor;
    }
    ...
    for (let i = 0; i < fenceBoards.length; i++) {
      if (
        ...
        fencesArr[i].numOfBoards < 7
      ) {
        fenceBoards[i][6].isVisible = false;
        inlays[i][0].isVisible = false;
        inlays[i][1].isVisible = false;
        inlays[i][2].isVisible = false;
        inlays[i][3].isVisible = false;
        fencesArr[i].inlays = 0;
      } else {
        if (fencesArr[i].fenceGardo == undefined) fencesArr[i].type = e;
      }
    }
  }
  var inlaysOn = 0;
        ...
        if (i == 0) {
          ...
          for (let i = 0; i < fenceBoards.length; i++) {
            if (
              ...
              fenceBoards[i][6].isVisible = true;
              inlays[i][0].isVisible = false;
              inlays[i][1].isVisible = false;
              inlays[i][2].isVisible = false;
              inlays[i][3].isVisible = false;
              fencesArr[i].inlays = 0;
              if (fencesArr[i].fenceGardo == undefined)
                fencesArr[i].type = "easyFence";
            }
          }
        } else if (i == 1) {
          inlaysFunction(1, 2, 3, silberMat, "easyFenceInlaysSilber");
        } else if (i == 2) {
          inlaysFunction(1, 2, 3, anthrazitMat, "easyFenceInlaysAnth");
        } else if (i == 3) {
          inlaysFunction(2, 1, 3, silberMat, "easyFenceInlaysSilber");
        } else if (i == 4) {
          inlaysFunction(2, 1, 3, anthrazitMat, "easyFenceInlaysAnth");
        } else if (i == 5) {
          inlaysFunction(3, 1, 2, silberMat, "easyFenceInlaysSilber");
        } else if (i == 6) {
          inlaysFunction(3, 1, 2, anthrazitMat, "easyFenceInlaysAnth");
        }
      });
    }
  }
      ...
      for (let i = 0; i < fencesArr.length; i++) {
        if (
          ...
          if (
            ...
            if (fencesArr[i].inlays == 1) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = silberMat;
              inlays[i][1].isVisible = true;
              inlays[i][2].isVisible = false;
              inlays[i][3].isVisible = false;
              inlays[i][1].material.albedoColor = silberMat.diffuseColor;
            }
            if (fencesArr[i].inlays == 2) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = anthrazitMat;
              inlays[i][1].isVisible = true;
              inlays[i][2].isVisible = false;
              inlays[i][3].isVisible = false;
              inlays[i][1].material.albedoColor = anthrazitMat.diffuseColor;
            }
            if (fencesArr[i].inlays == 3) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = silberMat;
              inlays[i][1].isVisible = false;
              inlays[i][2].isVisible = true;
              inlays[i][3].isVisible = false;
              inlays[i][2].material.albedoColor = silberMat.diffuseColor;
            }
            if (fencesArr[i].inlays == 4) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = anthrazitMat;
              inlays[i][1].isVisible = false;
              inlays[i][2].isVisible = true;
              inlays[i][3].isVisible = false;
              inlays[i][2].material.albedoColor = anthrazitMat.diffuseColor;
            }
            if (fencesArr[i].inlays == 5) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = silberMat;
              inlays[i][1].isVisible = false;
              inlays[i][2].isVisible = false;
              inlays[i][3].isVisible = true;
              // inlays[i][3].material.albedoColor =
              //   BABYLON.Color3.FromHexString("#e6e6e6");
            }
            if (fencesArr[i].inlays == 6) {
              inlays[i][0].isVisible = true;
              inlays[i][0].material = anthrazitMat;
              inlays[i][1].isVisible = false;
              inlays[i][2].isVisible = false;
              inlays[i][3].isVisible = true;
              // inlays[i][3].material.albedoColor =
              //   BABYLON.Color3.FromHexString("#474747");
            }
  ...
  function changeFenceFunction(a, b, c, d, e, f, g, h, k) {
    ...
    if (fencesArr[h].inlays == 0) {
      ...
      inlays[h][1].isVisible = false;
      inlays[h][2].isVisible = false;
      inlays[h][3].isVisible = false;
    }
    if (fencesArr[h].inlays > 0) {
      if (fencesArr[h].inlays == 1) {
        inlays[h][0].isVisible = true;
        inlays[h][0].material = silberMat;
        inlays[h][1].isVisible = true;
        inlays[h][2].isVisible = false;
        inlays[h][3].isVisible = false;
        inlays[h][1].material.albedoColor = silberMat.diffuseColor;
      }
      if (fencesArr[h].inlays == 2) {
        inlays[h][0].isVisible = true;
        inlays[h][0].material = anthrazitMat;
        inlays[h][1].isVisible = true;
        inlays[h][2].isVisible = false;
        inlays[h][3].isVisible = false;
        inlays[h][1].material.albedoColor = anthrazitMat.diffuseColor;
      }
      if (fencesArr[h].inlays == 3) {
        inlays[h][0].isVisible = true;
        inlays[h][0].mater = silberMat;
        inlays[h][1].isVisible = false;
        inlays[h][2].isVisible = true;
        inlays[h][3].isVisible = false;
        inlays[h][2].material.albedoColor = silberMat.diffuseColor;
      }
      if (fencesArr[h].inlays == 4) {
        inlays[h][0].isVisible = true;
        inlays[h][0].material = anthrazitMat;
        inlays[h][1].isVisible = false;
        inlays[h][2].isVisible = true;
        inlays[h][3].isVisible = false;
        inlays[h][2].material.albedoColor = anthrazitMat.diffuseColor;
      }
      if (fencesArr[h].inlays == 5) {
        inlays[h][0].isVisible = true;
        inlays[h][0].material = silberMat;
        inlays[h][1].isVisible = false;
        inlays[h][2].isVisible = false;
        inlays[h][3].isVisible = true;
        // inlays[h][3].material.albedoColor =
        //   BABYLON.Color3.FromHexString("#e6e6e6");
      }
      if (fencesArr[h].inlays == 6) {
        inlays[h][0].isVisible = true;
        inlays[h][0].material = anthrazitMat;
        inlays[h][1].isVisible = false;
        inlays[h][2].isVisible = false;
  ...
  let createLaisneSingle = (laisnePos) => {
    ...
    if (laisnePos < 6) {
      ...
      inlays[activeFence][1].position.y += 0.01;
      inlays[activeFence][2].position.y += 0.01;
      inlays[activeFence][3].position.y += 0.01;
    }
    for (let i = laisnePos; i < 7; i++) {
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          inlays[activeFence][1].isVisible = false;
          inlays[activeFence][2].isVisible = false;
          inlays[activeFence][3].isVisible = false;
        }
      ...
      if (
        ...
        if (
          ...
          if (fencesArr[activeFence].inlays == 1) {
            ...
            inlays[activeFence][0].material = silberMat;
            inlays[activeFence][1].isVisible = true;
            inlays[activeFence][2].isVisible = false;
            inlays[activeFence][3].isVisible = false;
            inlays[activeFence][1].material.albedoColor =
              silberMat.diffuseColor;
          }
          if (fencesArr[activeFence].inlays == 2) {
            inlays[activeFence][0].isVisible = true;
            inlays[activeFence][0].materia = anthrazitMat;
            inlays[activeFence][1].isVisible = true;
            inlays[activeFence][2].isVisible = false;
            inlays[activeFence][3].isVisible = false;
            inlays[activeFence][1].material.albedoColor =
              anthrazitMat.diffuseColor;
          }
          if (fencesArr[activeFence].inlays == 3) {
            inlays[activeFence][0].isVisible = true;
            inlays[activeFence][0].material = silberMat;
            inlays[activeFence][1].isVisible = false;
            inlays[activeFence][2].isVisible = true;
            inlays[activeFence][3].isVisible = false;
            inlays[activeFence][2].material.albedoColor =
              silberMat.diffuseColor;
          }
          if (fencesArr[activeFence].inlays == 4) {
            inlays[activeFence][0].isVisible = true;
            inlays[activeFence][0].material = anthrazitMat;
            inlays[activeFence][1].isVisible = false;
            inlays[activeFence][2].isVisible = true;
            inlays[activeFence][3].isVisible = false;
            inlays[activeFence][2].material.albedoColor =
          ...
          if (fencesArr[activeFence].inlays == 5) {
            inlays[activeFence][0].isVisible = true;
            inlays[activeFence][0].material = silberMat;
            inlays[activeFence][1].isVisible = false;
            inlays[activeFence][2].isVisible = false;
            inlays[activeFence][3].isVisible = true;
            // inlays[activeFence][3].material.albedoColor =
            //   BABYLON.Color3.FromHexString("#e6e6e6");
          }
          if (fencesArr[activeFence].inlays == 6) {
            inlays[activeFence][0].isVisible = true;
            inlays[activeFence][0].material = anthrazitMat;
            inlays[activeFence][1].isVisible = false;
            inlays[activeFence][2].isVisible = false;
  ...
  shareBut.onclick = () => {
    ...
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps.push([
        wholeFences[fencesArr[i].id].position.x,
        wholeFences[fencesArr[i].id].position.z,
        wholeFences[fencesArr[i].id].rotation.y,
        fencesArr[i].type,
      ]);
      sharedFences.push(i);
    }

    // push post color shaobj 4
    let postColor;
    if (allPosts[1].material.diffuseColor.r > 0.3) {
      postColor = 0;
    } else {
      postColor = 1;
    }
    shareProps.forEach((elm) => {
      elm.push(postColor);
    });

    //post heignt shaobj 5
    let sharePostProps = [];
    let shareSturVordProps = [];
    let shareSturRuckProps = [];
    for (let i = 0; i < rightPosts.length; i++) {
      sharePostProps.push(rightPosts[i]);
      shareSturVordProps.push(sturmankersVorderseiteRight[i]);
      shareSturRuckProps.push(sturmankersRuckseiteRight[i]);
    }
    for (let i = 0; i < shareProps.length; i++) {
      if (sharePostProps[i].scaling.z == 0.999) {
        shareProps[i].push(5);
      }
      if (sharePostProps[i].scaling.z == 0.724) {
        shareProps[i].push(4);
      }
      if (sharePostProps[i].scaling.z == 0.524) {
        shareProps[i].push(3);
      }
      if (sharePostProps[i].scaling.z == 1.475) {
        shareProps[i].push(2);
      }
      if (sharePostProps[i].scaling.z == 1.2) {
        shareProps[i].push(1);
      }
      if (sharePostProps[i].scaling.z == 1) {
        shareProps[i].push(0);
      }
    }

    // no sturmanker 0, vord 1, ruck 2 shaobj 6
    for (let i = 0; i < sharePostProps.length; i++) {
      if (shareSturVordProps[i][0].isVisible) {
        shareProps[i].push(1);
      } else if (shareSturRuckProps[i][0].isVisible) {
        shareProps[i].push(2);
      } else {
        shareProps[i].push(0);
      }
    ...
    //share parent children shaobj 7 i 8
    for (let i = 0; i < sharedFences.length; i++) {
      if (i == 0) {
        shareProps[i].push(undefined, fencesArr[sharedFences[i]].children);
      } else {
        shareProps[i].push(
          fencesArr[sharedFences[i]].parent,
          fencesArr[sharedFences[i]].children
        );
      }
    }
    //start und absch color shaobj 9
    for (let i = 0; i < fencesArr.length; i++) {
      if (fencesArr[i].startUndAbschCol == "silber") {
        shareProps[i].push(0);
      } else {
        shareProps[i].push(1);
      }
    ...
    //boadrs color shaobj 10
    for (let i = 0; i < fencesArr.length; i++) {
      if (fencesArr[i].boardCol == "grau") {
        shareProps[i].push(0);
      }
      if (fencesArr[i].boardCol == "anthrazit") {
        shareProps[i].push(1);
      }
      if (fencesArr[i].boardCol == "braun") {
        shareProps[i].push(2);
      }
      if (fencesArr[i].boardCol == "sand") {
        shareProps[i].push(3);
      }
    }
    //laisne shaobj 11
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps[i].push(fencesArr[i].laisnes);
    }
    //laisne color shaobj 12
    for (let i = 0; i < fencesArr.length; i++) {
      if (fencesArr[i].laisnesCol == "silber") {
        shareProps[i].push(0);
      } else {
        shareProps[i].push(1);
      }
    ...
    //inlays shaobj 13
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps[i].push(fencesArr[i].inlays);
    }

    //led shaobj 14
    for (let i = 0; i < fencesArr.length; i++) {
      ledsRight[i].isVisible ? shareProps[i].push(1) : shareProps[i].push(0);
    }
    //single boards shaobj 15
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps[i].push(fencesArr[i].numOfBoards);
    }
    //small boards color shaobj 16
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps[i].push(fencesArr[i].smBoaCol);
    }
    //hauswand shaobj 17
    for (let i = 0; i < fencesArr.length; i++) {
      directeHauswandMeshesRight[i].isVisible
        ? shareProps[i].push(1)
        : shareProps[i].push(0);
    }
    //size shaobj 18
    for (let i = 0; i < fencesArr.length; i++) {
      shareProps[i].push(fencesArr[i].size);
    }
    ////
    /////////////////////////finale share props
    finalShareProps = [];
    for (let i = 0; i < shareProps.length; i++) {
      //share parent children
      shareProps[i].length = 19;
      ///////
      shareProps[i][0] = Math.round(shareProps[i][0] * 100) / 100;
      shareProps[i][1] = Math.round(shareProps[i][1] * 100) / 100;
      shareProps[i][2] = Math.round(shareProps[i][2] * 100) / 100;
      finalShareProps.push(shareProps[i]);
    }

    //main post props
    //main post size shaobj 19
    if (allPosts[0].scaling.y == 1) finalShareProps[0].push(0);
    if (allPosts[0].scaling.y == 1.2) finalShareProps[0].push(1);
    if (allPosts[0].scaling.y == 1.475) finalShareProps[0].push(2);
    if (allPosts[0].scaling.y == 0.524) finalShareProps[0].push(3);
    if (allPosts[0].scaling.y == 0.724) finalShareProps[0].push(4);
    if (allPosts[0].scaling.y == 0.999) finalShareProps[0].push(5);
    //main post sturmanker shaobj 20
    if (sturmankersVorderseite[0].isVisible) {
      finalShareProps[0].push(1);
    } else if (sturmankersRuckseite[0].isVisible) {
      finalShareProps[0].push(2);
    } else {
      finalShareProps[0].push(0);
    }
    //main post led shaobj 21
    leds[0].isVisible ? finalShareProps[0].push(1) : finalShareProps[0].push(0);

    //led color shaobj 22
    finalShareProps[0].push(ledMat.diffuseColor);

    //main hauswand shaobj 23
    directeHauswandMeshes[0].isVisible
      ? finalShareProps[0].push(1)
      : finalShareProps[0].push(0);
    ///////////////////////////////////
    data = JSON.stringify(finalShareProps);
    let propsEncoded = encodeURIComponent(data);
    // let shareLink = window.location.href + "?" + propsEncoded;
    let shareLink = window.parent.location.href + "?" + propsEncoded;
